...
| `PreToolUse` | `integrity-guard.sh` | 四权分立：拦截对 tests/evals/CI/secrets/hidden solutions 的写入或读取，deny 或 advisory |
...
| 四权分立 (integrity guard) | ✅ integrity-guard.sh | ✖ | ✅ | 对标 Claude plugin 的 PreToolUse |
...
- 四权分立只做 advisory/deny 通知，不做权限管理系统。
