...
- 新一代前沿模型（Anthropic Opus 4.5 / 4.6+）被明确训练为抵抗谄媚与情感操纵，并建立指令层级（系统提示 > 用户输入）与提示注入防御，PUA 式话语施压会被模型当作「待评估的内容」而非「必须服从的权威」主动过滤或反驳（官方审计显示 Opus 4.5 家族谄媚率比 Opus 4.1 低 70–85%；Opus 4.8 甚至出现过度防御，见 anthropics/claude-code #67606）。
- 上游 `tanweai/pua` 无按模型的兼容性矩阵（兼容性按平台划分），但社区普遍观察到 Opus 系列对施压式话术顺从度低（PUA「不触发 / 不表演」）；且 Opus 4.8 的工具调用序列化缺陷（#67307、#63481）会破坏依赖 `tool_result` / `before_agent_start` 等 hook 的确定性链路——工程层失效。
- 高阶模型对 PUA 的依赖本身也在下降（推论）：高自主性（不放弃、穷尽方案、验证后才宣称完成）已是默认训练行为，外部施压增益趋近于零，只是噪音。
- 因此该能力定位为**按需开关**：在能获得行为增益的模型上保持开启，对既不响应施压也不需要它的高阶模型直接排除，避免无效注入（token 浪费 + 信任损耗）。
