...
`pi install` 是 pi 内置的包管理命令，支持 npm、git 和本地路径：

```bash
# ▸ 从 npm registry 安装（全局，推荐）
...
# ▸ 项目级安装（团队共享，写入 .pi/settings.json）
pi install -l npm:@xnightsky/pi-pua-x

# ▸ 卸载
pi remove npm:@xnightsky/pi-pua-x

# ▸ 临时试用（不写入 settings）
pi -e npm:@xnightsky/pi-pua-x
```

安装后**重启 pi** 即自动加载，无需手动复制文件。

> ⚠️ **切换安装来源前必须先卸载旧版本**：如果之前是用 `git:` 装的，换到 `npm:` 前
> 必须 `pi remove git:github.com/xnightsky/pi-pua-x`，否则 pi 里同时挂了两个同名扩展，
> 命令和钩子会重复注册。
...
```bash
# 更新到最新
pi update npm:@xnightsky/pi-pua-x

# 或重新 install 指定版本
pi install npm:@xnightsky/pi-pua-x@0.4.0
```
...
| `/pua-model disable <pattern>` | 禁用 PUA（如 `anthropic/claude-opus*`） |
...
| npm 包安装 404 | 新旧安装来源冲突 | 先 `pi remove` 旧版本（`git:` 或 `npm:`），再装新版本 |
...
解决：在 https://www.npmjs.com/settings/{username}/tokens 生成 **Granular Access Token**，
勾选 **Bypass 2FA when publishing**，然后用 `npm config set //registry.npmjs.org/:_authToken=npm_xxxxx` 设置。
...
作用域包（`@scope/pkg`）publish 默认需要付费计划。解决：
...
OTP 验证码只有 30 秒有效期。从认证器读到后要立即执行，过期了重看一个。
...
```
# 之前用 git: 装的，换成 npm: 时
pi remove git:github.com/xnightsky/pi-pua-x   # 先删旧的
pi install npm:@xnightsky/pi-pua-x             # 再装新的
```

如果不删直接装，pi 里会同时存在两个同名扩展的注册记录，命令报重复注册错误。

**坑5：作用域包安装可能因缓存 404**

首次发布后立即 `pi install` 可能 404（npm 索引尚未完全同步）。等 1-2 分钟重试，或指定版本号：
